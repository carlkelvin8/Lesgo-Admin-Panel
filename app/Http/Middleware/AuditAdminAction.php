...
class AuditAdminAction
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $oldSnapshot = $this->shouldCaptureOldValues($request) && $resource instanceof Model
            ? $this->sanitize($resource->getAttributes())
            : null;
        ...
        try {
            ...
            $newValues = $this->sanitize($request->except([
    ...
    /**
     * Recursively strip any value that cannot be JSON-encoded (UploadedFile,
     * objects, resources, etc.) — replace them with a safe placeholder string
     * so the audit log never crashes on file upload or complex request payloads.
     */
    protected function sanitize(mixed $value): mixed
    {
        ...
        // UploadedFile, objects, resources → store a descriptive placeholder
