...
<div class="max-w-4xl space-y-6">
    <article class="bg-white rounded-xl shadow-sm p-8"><div class="flex gap-2 mb-4"><span class="px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs">{{ $article->category->name ?? 'Uncategorized' }}</span><span class="px-2 py-1 rounded text-xs {{ $article->is_published ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">{{ $article->is_published ? 'Published' : 'Draft' }}</span></div><h1 class="text-2xl font-bold mb-3">{{ $article->title }}</h1>@if($article->excerpt)<p class="text-gray-500 mb-6">{{ $article->excerpt }}</p>@endif<div class="text-gray-800 whitespace-pre-wrap leading-7">{{ $article->content }}</div></article>
